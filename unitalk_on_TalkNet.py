...
parser.add_argument('--predictions_dir',       type=str, default="predictions", help='Path for the output predictions csv file')
...
def main():
	...
	# TODO: batch infer is unfair - but it might just not be possible to make a fair comparison because TalkNet is just conceptuallly different and not designed for realTime application. Maybe we can just report the time for each step and the total time, and compare with the real-time requirement (e.g., 25 fps) to see if it can be used for real-time application.
	print("Creating the folder for the predictions in %s"%(args.predictions_dir))
	os.makedirs(args.predictions_dir, exist_ok = True) # The path to the predictions dir
	print("Start processing the video in %s"%(args.videoFolder))
	for video in glob.glob(args.videoFolder + '/*.*'):
		# record timeing
		t0 = time.time()
		if Path(video).suffix in ['.mp4', '.avi', '.mkv', '.mpeg', '.mp3', '.wav']:
			args.videoName = str(Path(video).stem)
			args.videoPath = str(Path(video))
			args.savePath = os.path.join(args.videoFolder, args.videoName)
		...
		files.sort()
		scores, native_fps = evaluate_network(files, args)
		# scores seems to be a list of scores per frames per face track - for each face track there is an array containing numbers corresponding to the speech activity of that frame
		# We assume bboxes are pixel values of the original video
		# allTracks is a list of dicts with keys 'frame' and 'bbox' - the frame key contains an array of frames that the track is present in, and the bbox key contains an array of bounding boxes corresponding to those frames.
	

		# for track, score in zip(allTracks, scores):
		# 	if track['frame'].shape[0] != len(score):
		# 		print("Warning: the track frames and score frames do not match, please check the code!")
		# 		print(track['frame'].shape[0], len(score))
		# 	if track['bbox'].shape[0] != len(score):
		# 		print("Warning: the track bounding boxes and score frames do not match, please check the code!")
		# 		print(track['bbox'].shape[0], len(score))

		## Always one score missing - is it the first or the last one? Any hints? No - zip just handels this by ignoring the last one.
		predictions_csv = os.path.join(args.predictions_dir, args.videoName + '.csv')
		...
		video.release()
		append_aggregate_time(os.path.join(args.predictions_dir, 'aggregate_time.csv'), args.videoName, elapsed, total_frames, native_fps)
